...
- `develop`
  - 日常集成分支
  - 只接收工作分支合并结果
  - 不承载当前功能开发，不在此分支上直接改代码或提交日常开发内容
- `feature/<scope>-<topic>`
  - 新功能分支
  - 从 `develop` 拉出
- `fix/<scope>-<topic>`
...
2. 如果是常规开发任务，从 `develop` 拉工作分支，在 `feature/*`、`fix/*` 或 `chore/*` 上完成开发
3. 如果当前所在分支不符合规则，先切分支，再开始改代码

明确约束：

- 当前功能开发必须在功能分支上进行，不能直接在 `develop` 上实现
- `develop` 的职责是集成、联调、验收，不是日常编码分支
...
- 当前功能开发的 commit 必须落在 `feature/*`、`fix/*`、`chore/*` 或 `hotfix/*`
...
- 开发统一从 `develop` 开始
- 开发统一从 `develop` 拉出工作分支，而不是停留在 `develop` 上直接写代码
